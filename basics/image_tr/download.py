...
def download_and_expand_image(url: str, data_dir: str) -> None:
    """
    Given a URL and the image data directory, fetch the URL, save it in the data directory,
    expand the image to 4 times its original size, and remove the original image.
    """
    Image.MAX_IMAGE_PIXELS = None  # Remove the pixel limit entirely
    img_data = requests.get(url).content
    img_name = url.split("/")[4]
    img_name = f"{data_dir}/{img_name}"
